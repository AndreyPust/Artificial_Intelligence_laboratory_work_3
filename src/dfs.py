...
class Node:
    ...
    def __lt__(self, other):
        """Сравнение узлов (для приоритетных очередей), здесь необязательно."""
...
def expand(problem, node):
    """
    Генерируем (расширяем) дочерние узлы, применяя все действия к состоянию node.state.
    """
    s = node.state
...
def is_cycle(node):
    """
    Функция, которая проверяет на зацикливание.
    :param node: Узел.
...
def depth_first_recursive_search(problem, node=None):
    ...
    Если зациклились или не нашли решения по всем ветвям, возвращаем failure.
    """
